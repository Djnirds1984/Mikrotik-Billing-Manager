...
app.post('/api/upload-restore', protect, express.raw({ type: 'application/octet-stream', limit: '50mb' }), (req, res) => {
    res.setHeader('Content-Type', 'text/event-stream');
    const send = (data) => res.write(`data: ${JSON.stringify(data)}\n\n`);
    ...
    const restore = async () => {
        try {
            ...
            const safetyBackupPath = DB_PATH + `.bak-${Date.now()}`;
            send({ log: `Creating safety backup of current database...` });
            await fs.promises.copyFile(DB_PATH, safetyBackupPath);

            send({ log: 'Overwriting database with uploaded file...'});
            await fs.promises.writeFile(DB_PATH, req.body);

            send({ log: 'Restarting panel service... The panel will reload shortly.'});
            exec('pm2 restart mikrotik-manager', (err, stdout, stderr) => {
                if (err) {
                    send({ log: `ERROR: PM2 restart failed: ${stderr || err.message}`, isError: true });
                    send({ status: 'error', message: err.message });
                } else {
                    send({ log: stdout });
                    send({ status: 'restarting' });
