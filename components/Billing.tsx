...
import type { RouterConfigWithId, BillingPlan, BillingPlanWithId, PppProfile } from '../types.ts';
...
const PlanFormModal: React.FC<{
    ...
    plan: BillingPlanWithId | null;
    profiles: PppProfile[];
    isLoading: boolean;
}> = ({ isOpen, onClose, onSave, plan, profiles, isLoading }) => {
    const { currency } = useLocalization();
    const [formState, setFormState] = useState<BillingPlan>({
        name: '',
        price: 0,
        cycle: 'Monthly',
        pppoeProfile: '',
        ...
        if (isOpen) {
            if (plan) {
                ...
                setFormState({
                    ...
                    price: 0,
                    cycle: 'Monthly',
                    pppoeProfile: profiles.length > 0 ? profiles[0].name : '',
                    description: '',
                    currency: currency,
                });
            }
        }
    }, [isOpen, plan, profiles, currency]);
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
            <div className="bg-white dark:bg-slate-800 rounded-lg shadow-xl w-full max-w-lg">
                <form onSubmit={handleSubmit}>
                    <div className="p-6">
                        <h3 className="text-xl font-bold mb-4">{plan ? 'Edit' : 'Add'} Billing Plan</h3>
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div><label>Cycle</label><select name="cycle" value={formState.cycle} onChange={handleChange} className="mt-1 w-full p-2 bg-slate-100 dark:bg-slate-700 rounded-md"><option>Monthly</option><option>Quarterly</option><option>Yearly</option></select></div>
                            </div>
                            <div><label>PPPoE Profile</label><select name="pppoeProfile" value={formState.pppoeProfile} onChange={handleChange} className="mt-1 w-full p-2 bg-slate-100 dark:bg-slate-700 rounded-md">{profiles.map(p => <option key={p.id} value={p.name}>{p.name}</option>)}</select></div>
                            <div><label>Description</label><textarea name="description" value={formState.description} onChange={handleChange} rows={2} className="mt-1 w-full p-2 bg-slate-100 dark:bg-slate-700 rounded-md" /></div>
                        </div>
                    </div>
...
export const Billing: React.FC<{
    ...
    profiles: PppProfile[];
}> = ({ selectedRouter, profiles }) => {
    const { plans, addPlan, updatePlan, deletePlan, isLoading, error } = useBillingPlans(selectedRouter?.id || null);
    ...
    const handleOpenEditModal = (plan: BillingPlanWithId) => {
        setEditingPlan(plan);
        setIsModalOpen(true);
    };
    ...
    if (!selectedRouter) {
        return <div className="p-4 text-center text-slate-500">Please select a router to manage billing plans.</div>;
    }
    ...
    return (
        <div className="space-y-6">
            <PlanFormModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onSave={handleSave} plan={editingPlan} profiles={profiles} isLoading={isSubmitting} />

            <div className="flex justify-between items-center">
                <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-200 flex items-center gap-2"><SignalIcon className="w-6 h-6" /> PPPoE Billing Plans</h2>
                    ...
                    <table className="w-full text-sm">
                        <thead className="text-xs uppercase bg-slate-50 dark:bg-slate-900/50">
                            <tr>
                                ...
                                <th className="px-6 py-3">PPPoE Profile</th>
                        ...
                        <tbody>
                            {plans.map(plan => (
                                <tr key={plan.id} className="border-b dark:border-slate-700">
                                    ...
                                    <td className="px-6 py-4 font-mono">{plan.pppoeProfile}</td>
