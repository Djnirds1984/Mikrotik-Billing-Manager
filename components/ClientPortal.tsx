...
import { BirInfoBlock, birHtmlLine } from './BirInfo.tsx';
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setPanelSettings({ companyName: company.companyName || '', logoBase64: company.logoBase64 || '', address: (company as any).address || '', contactNumber: (company as any).contactNumber || '', email: company.email || '', tinNumber: (company as any).tinNumber || '', registeredBusinessName: (company as any).registeredBusinessName || '', businessStyleName: (company as any).businessStyleName || '', dtiSecRegNo: (company as any).dtiSecRegNo || '', businessPermitNo: (company as any).businessPermitNo || '', vatType: (company as any).vatType || '', birStatement: (company as any).birStatement || '' });
      } catch {
        ...
        {invoiceToPrint && (
          <div className="p-8 font-sans text-black bg-white">
            <header className="flex justify-between items-start pb-4 border-b-2 border-black">
              ...
              <BirInfoBlock companySettings={panelSettings} />
            </div>
