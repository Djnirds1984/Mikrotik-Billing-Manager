...
export const Help: React.FC<{
    ...
    return (
        <div className="bg-white dark:bg-slate-800 rounded-lg shadow-xl w-full h-full border border-slate-200 dark:border-slate-700 flex flex-col" style={{ minHeight: 'calc(100vh - 140px)' }}>
            <header className="p-4 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center">
                <div>
                    <h3 className="text-xl font-bold text-[--color-primary-500] dark:text-[--color-primary-400]">AI Assistant</h3>
                    <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Chat with AI to get help with your tasks</p>
                </div>
                <button 
                    onClick={handleGenerateReport} 
                    disabled={isReporting} 
                    className="px-4 py-2 text-sm bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-300 rounded-lg disabled:opacity-50"
                >
            ...
            <div ref={chatContainerRef} className="flex-1 p-4 overflow-y-auto space-y-4">
                {history.map((msg, index) => (
                    <div key={index} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                        <div className={`max-w-2xl p-4 rounded-2xl ${
                            msg.role === 'user' 
                                ? 'bg-[--color-primary-600] text-white rounded-br-sm' 
                                : 'bg-slate-100 dark:bg-slate-700 text-slate-800 dark:text-slate-200 rounded-bl-sm'
                        }`}>
                            <p className="text-sm whitespace-pre-wrap">
                                {msg.content.replace(/\*\*(.*?)\*\*/g, '<strong class="font-bold">$1</strong>')
                                    .split('\n')
                ...
                {isLoading && (
                    <div className="flex justify-start">
                        <div className="max-w-2xl p-4 rounded-2xl rounded-bl-sm bg-slate-100 dark:bg-slate-700">
                            <Loader />
                        </div>
                    </div>
                )}
            ...
            <footer className="p-4 border-t border-slate-200 dark:border-slate-700">
                <div className="flex items-end gap-3">
                    <textarea
                        ...
                        className="flex-1 p-3 bg-slate-100 dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-lg text-slate-900 dark:text-white resize-none focus:outline-none focus:ring-2 focus:ring-[--color-primary-500]"
                        rows={2}
                        disabled={isLoading}
                    />
                    <button 
                        ...
                        className="px-6 py-3 bg-[--color-primary-600] hover:bg-[--color-primary-500] rounded-lg disabled:opacity-50 text-white font-medium transition-colors"
                    >
                        Send
                    </button>
                </div>
